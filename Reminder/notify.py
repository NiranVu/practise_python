...
def task_n_session():
    ...
    toaster.show_toast("Pomodoro Clock started!!", "Let's do it :)")
...
def pomodoro_clock(task, session):
    ...
    if session_value < 4:
        ...
        for i in range(session_value):
            time.sleep(1200)
            toaster.show_toast("Pomodoro Notifications!!", "Break time :)")
            time.sleep(300)
            toaster.show_toast("Pomodoro Notifications!!", "Back to work xD")
    ...
    # for every fourth clock the break time will be 20 minutes
        ...
        for i in range(session_value):
            if j % 4 != 0:
                ...
                time.sleep(1200)
                toaster.show_toast(
                    "Pomodoro Notifications!!", "Take 20min Break time :)"
                )
                time.sleep(1200)
                toaster.show_toast("Pomodoro Notifications!!", "Back to work xD")
            j += 1
